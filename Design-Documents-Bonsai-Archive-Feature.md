...
For background, there is a design document relating to Bonsai DB in the wiki: [Bonsai Tries Design Overview](Design-Documents-Bonsai-Tries-Design-Overview).

The issue for Bonsai archive is [besu-eth/besu#5846](https://github.com/besu-eth/besu/issues/5846) and the PR is [besu-eth/besu#7475](https://github.com/besu-eth/besu/pull/7475). For context, there was originally a [draft PR](https://github.com/besu-eth/besu/pull/5865) which was a start at delivering the feature, and a re-based branch of that PR at [jframe/besu multi-version-flat-db-rebase](https://github.com/jframe/besu/tree/multi-version-flat-db-rebase).
...
- `CODE_STORAGE`
  - Key = hash of code or hash of account (based on the `-Xbonsai-code-using-code-hash-enabled` config option)
  - Value = the code
...
Currently, the column families are identified by an individual byte. You need to know that e.g. `0x0a` maps to `TRIE_LOG_STORAGE` in order to make direct queries of the database for problem diagnosis. For example, `ldb --db=. --column_family=$'\x0a' ...` is used to perform queries against the `TRIE_LOG_STORAGE` DB segment. The 2 new archive segments have been named with their full name. This makes direct DB queries much easier, as per the example given above querying `--column_family=ACCOUNT_INFO_STATE_ARCHIVE`. It may be useful in the future to change the existing DB segments to use their full names, but since this would require DB migration for existing state, it has not been done as part of the archive work.

...
Example warning output should include a clear `WARNING` label and a one-line explanation.
...
Example error output should include a clear `ERROR` label and guidance on recovery where possible.
...
For unknown option errors, show the invalid option clearly and include a "Similar options" suggestion list when close matches exist.
...
Use warnings for non-fatal states where user action may still be required.
...
Use errors for failed actions where the requested operation could not be completed.
...
Use warning logs for degraded or potentially harmful states.
...
Use failure-level messaging when a command terminates without completing its intent.

...
It's dawned on us that Besu now represents upward of **~$5B on Mainnet in stake**! Great job everyone — [https://supermajority.info/](https://supermajority.info/)
...
- Ensure test coverage for key features e.g. bonsai.
...
- An Epic/workstream is created with actionable items based on the findings from the brainstorm.
...
- Consensus issue
- Sync issue
- Peering issue (e.g. Besu nodes lose 100% of peers and drop off the network)
- Chain fork
...
- Pre-release
  ...
  - See the Testing page. <!-- TODO: link to Testing page once ported from confluenceExport -->
...
See the Testing Gaps Epic for the stack ranked issues. (For those without Zenhub licences, the epic is [here](https://github.com/hyperledger/besu/issues/6538) in GitHub but it won't show all the detail.)
...
- Downgrade test — would be good to have a regression test for each release to check we can downgrade to previous version(s).
- Automate burn-in.
...
- Decouple ATs from mainnet.json.
  - Most acceptance tests use mainnet.json, so bugs on adding a new fork (e.g. cancun) may not get covered until much later in the SDLC.
...
- Code coverage aggregation [https://github.com/hyperledger/besu/issues/6650](https://github.com/hyperledger/besu/issues/6650)
  ...
  - And consider making it a condition for merging a PR?
- Snap gaps!

...
> [!NOTE]
> This is a historical design document from Q1 2024 proposing changes to Besu defaults and the introduction of profiles. It is preserved as a record of the proposal.
...
For private network users, configuration must be done either way to set up a custom genesis, chain ID, and to configure connections to peers. Defaults will have less impact on these users. Enterprises and developers are more likely to read the documentation prior to first startup.
...
- Data-storage-format: BONSAI
  - Full flat DB by default (`Xsnapsync-synchronizer-flat-db-healing-enabled=true` — delete this flag, per safety concerns raised by Gary Schulte)
